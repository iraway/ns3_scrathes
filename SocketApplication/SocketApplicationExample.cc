...
int main(int argc, char* argv[])
{
    ...
    Ipv4Address dest_ip0 = interfaces0.GetAddress(0) __attribute__((unused));;
    Ipv4Address dest_ip2 = interfaces1.GetAddress(1);

    Ptr<Packet> packet0 = Create<Packet>(16);

    Simulator::Schedule(Seconds(2), &SocketApplication::SendPacket, udp0, packet0, dest_ip2, port);

    //Simulator::Schedule(Seconds(4), &SocketApplication::SendPacket, udp2, packet0, dest_ip0, port);

    AsciiTraceHelper ascii;
    pointToPoint0.EnableAsciiAll(ascii.CreateFileStream("pcap/SocketApplication/full.tr"));
    pointToPoint1.EnableAsciiAll(ascii.CreateFileStream("pcap/SocketApplication/full.tr"));

    pointToPoint0.EnablePcapAll("pcap/SocketApplication/full");
    pointToPoint1.EnablePcapAll("pcap/SocketApplication/full");

    LogComponentEnable("SocketApplication", LOG_LEVEL_ALL);
